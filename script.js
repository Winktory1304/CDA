...
function displaySchedule(rounds) {
    ...
    rounds.forEach((round, roundIndex) => {
        ...
        round.matches.forEach((match, matchIndex) => {
            ...
            // Vorbelegung falls schon Ergebnis vorhanden
            legsInput1.value = match.legs ? match.legs.split("-")[0] : "";
            ...
            legsInput2.value = match.legs ? match.legs.split("-")[1] : "";

            // Eventlistener für beide Felder – bei Änderung wird das Ergebnis aktualisiert
            ...
            // Füge die Inputfelder und einen Trenner (z.B. ":") in die Anzeige ein
...
function updateMatchResult(roundIndex, matchIndex) {
    ...
    const score1 = input1.value.trim();
    const score2 = input2.value.trim();

    if (score1 === "" || score2 === "") {
        tournamentData.rounds[roundIndex].matches[matchIndex].legs = "";
    } else {
...
// Berechnet und aktualisiert den Turnierstand inkl. Zählung der Legs
function updateStandings() {
    ...
    tournamentData.rounds.forEach(round => {
        round.matches.forEach(match => {
            ...
            if (score1 > score2) {
                ...
                standings[player1].points += 2;
            } else if (score2 > score1) {
                standings[player2].wins += 1;
                standings[player1].losses += 1;
                standings[player2].points += 2;
            } else {
                standings[player1].draws += 1;
                standings[player2].draws += 1;
                standings[player1].points += 1;
                standings[player2].points += 1;
            }
        });
    });
    saveTournament();
    ...
    standingsArray.sort((a, b) => {
        ...
        let diffA = a.legsFor - a.legsAgainst;
        let diffB = b.legsFor - b.legsAgainst;
        return diffB - diffA;
    });
    const standingsDiv = document.getElementById("standingsList");
    standingsDiv.innerHTML = "";
    const table = document.createElement("table");
    const headerLabels = ["Spieler", "Spiele", "Siege", "Unentsch.", "Niederl.", "Legs", "Gegenlegs", "Punkte"];
